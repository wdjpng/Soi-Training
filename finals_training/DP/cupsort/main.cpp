...
signed main() {
    ...
    for (int i = 0; i < n; ++i) {
        int numberOfCups, group;
        cin >> numberOfCups >> group;

        numberOfInversions += numberOfCups * segtree.query(group + 1, size).sum;
        segtree.update(group, Value({segtree.tree[segtree.n + group].sum + numberOfCups}));
    }
